...
> Historical candidate: superseded by run `29143013811`. This file remains an
> integrity record for the earlier artifact and is not the current release-gate
> evidence.
...
The earlier screenshot review was not persisted with a machine-readable reviewer
identity, timestamp, rubric, and screenshot digest, so this historical attestation
does not treat that review as durable release evidence. The superseding run
`29143013811` records those fields in
`windows-x64-installed-app-29143013811-visual-review.json`.

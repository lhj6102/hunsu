...
1. Discover an available Runner capability, select an existing commit as the Project root, and explicitly confirm its initial Node Plan.
...
4. Compare completed results through an explicit sibling-Run or coached-How cohort, then explicitly select or reject result Nodes without merging their structural lineage.
...
2. Every non-root Node has one structural parent. Branching preserves alternatives; comparison and decisions are non-structural decorations, and Hunsu never invents convergence edges.
...
Runner types remain open and registry-driven. Before a Project chooses its first How or Coaching changes How, the product discovers an exact available Runner capability and schema. Player and Team are useful bundled examples, not a fixed mode switch.

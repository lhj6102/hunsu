...
`0.2.0-next.6`. npm versions are immutable. CI never invents or rewrites a
version.
...
~~~sh
git tag -a v0.2.0-next.6 <approved-preview-sha> \
  -m "Release @hunsu/bridge 0.2.0-next.6 candidate"
git push origin v0.2.0-next.6
~~~
...
~~~sh
npm dist-tag add @hunsu/bridge@0.2.0-next.6 next

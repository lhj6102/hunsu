...
`0.2.0-next.3`. npm versions are immutable. CI never invents or rewrites a
version.
...
~~~sh
git tag -a v0.2.0-next.3 <approved-preview-sha> \
  -m "Release @hunsu/bridge 0.2.0-next.3 candidate"
git push origin v0.2.0-next.3
~~~
...
~~~sh
npm dist-tag add @hunsu/bridge@0.2.0-next.3 next

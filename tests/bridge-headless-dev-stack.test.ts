...
test("npm provenance verification binds the signed package subject to the protected preview publisher and exact Git SHA", async () => {
  ...
  const untrustedRegistryAudit = structuredClone(audit);
  untrustedRegistryAudit.verified[0].registry = "https://registry.npmjs.org.example/";
  assert.throws(() => provenance.verifyBridgeNpmProvenanceAudit({
    audit: untrustedRegistryAudit,
    version,
    versionTag,
    gitSha,
    integrity
  }), /unexpected registry/u);

...
test("Linux service install writes a safely escaped direct-daemon unit and never starts it", async () => {
  ...
  assert.match(unit, /^ExecStart="\/opt\/Hunsu 100%%\/node" "\/opt\/Hunsu 100%%\/runtime\/hunsu-bridge\.js" "daemon" "--runtime-path" "\/opt\/Hunsu 100%%\/runtime" "--profile" "preview"$/mu);
...
test("offline service status combines manager, health, authentication, version, and runtime path", async () => {
  ...
  const manager = createLinuxSystemdUserServiceManager({
    ...
    readInstalledRuntime: async () => ({
      packageVersion: "0.2.0-next.5",
      runtimePath: "/home/test/.local/share/hunsu/bridge/runtime/versions/0.2.0-next.5",
      deploymentProfile: "preview"
    })
  });
...
test("macOS LaunchAgent is a direct daemon with RunAtLoad, crash-only KeepAlive, escaped paths, and logs", async () => {
  ...
  assert.match(plist, /<string>--profile<\/string>\s*<string>preview<\/string>/u);
...
test("macOS reloads a changed cached LaunchAgent definition before starting the new stable CLI", async () => {
  ...
  const result = await manager.install({
    ...
    cliPath: "/Users/test/.hunsu/runtime/versions/0.2.0-next.5/dist/cli.js",
    hunsuHome: "/Users/test/.hunsu",
    packageVersion: "0.2.0-next.5",
    runtimePath: "/Users/test/.hunsu/runtime/versions/0.2.0-next.5",
...
test("Windows Task Scheduler uses current-user ScheduledTasks, hidden settings, and absolute direct paths", async () => {
  ...
  assert.match(script, /--profile preview/u);
...
test("Windows service install reports changes when the stable action contract changes", async () => {
  const input: ServiceInstallInput = {
    nodePath: "C:\\Program Files\\nodejs\\node.exe",
    cliPath: "C:\\Users\\test\\Hunsu\\runtime\\0.2.0-next.5\\cli.js",
    hunsuHome: "C:\\Users\\test\\Hunsu",
    packageVersion: "0.2.0-next.5",
    runtimePath: "C:\\Users\\test\\Hunsu\\runtime\\0.2.0-next.5",
    deploymentProfile: "preview"
  };
  ...
  existingArguments = `"C:\\old\\cli.js" daemon --runtime-path "${input.runtimePath}" --home "${input.hunsuHome}" --profile preview`;
